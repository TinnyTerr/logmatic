	...
	private logs: {
		...
		funcs: async (level, ...data) => {
			for (let index = 0; index < this.funcs.length; index++) {
				...
				try {
					element(this.options.levels[level], ...data);
				} catch (err) {
					this.internalLogging(
 ...
 */
type logFunction = (level: customLevel, ...data: any[]) => void;
